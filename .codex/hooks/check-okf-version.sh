#!/usr/bin/env bash
# Destination in your repo: .codex/hooks/check-okf-version.sh
# No chmod needed: hooks.json invokes this via `bash`.
#
...
# either differs, it injects a note into Codex's context; the "OKF version
# policy" and "Kit version policy" sections in AGENTS.md tell Codex what to do.
...
if [ -n "$latest" ] && [ "$latest" != "$declared" ]; then
  notes="OKF version check: the latest OKF spec version on the official main branch is $latest. This repo's docs/index.md declares okf_version ${declared:-(none)}. The OKF version policy in AGENTS.md applies."
fi
...
if [ -n "$kit_declared" ]; then
  ...
  if [ -n "$kit_latest" ] && [ "$kit_latest" != "$kit_declared" ]; then
    kit_note="Kit version check: this repo was installed from claude-okf-repo-kit $kit_declared and the published kit version is $kit_latest. The Kit version policy in AGENTS.md applies."
    if [ -n "$notes" ]; then
