#!/usr/bin/env bash
# Destination in your repo: .codex/hooks/check-docs-sync.sh
# No chmod needed: hooks.json invokes this via `bash`.
#
# Stop hook: when Codex tries to finish a turn, this first checks whether
...
if [ -n "$code_changed" ] && [ -z "$docs_changed" ]; then
  cat <<'EOF'
{"decision": "block", "reason": "Code changed this session but nothing under /docs was updated. Per the AGENTS.md workflow: if behavior or a contract changed, update the governing file in /docs/specs or /docs/adr and add a dated entry to /docs/log.md. If no doc change is warranted, add a one-line entry to /docs/log.md saying why."}
EOF
  exit 0
fi
