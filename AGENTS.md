...
Codex does not resolve `@` imports. At session start, before the first task, read these files so the goal and the knowledge indexes are in context. Keep them small; full specs and ADRs stay on disk until a task needs them.
...
- `.codex/hooks.json` — project hook wiring for Codex: the same SessionStart and Stop guardrail hooks that `.claude/settings.json` wires for Claude Code, with scripts resolved from the git root. Committed. Codex loads it only when the project's `.codex` layer is trusted.
- `.claude/skills/okf-*/SKILL.md` — kit-installed workflow skills (goal interview, acceptance pass, ADR review, kit upgrade, adoption pass, second-agent port) for Claude Code sessions. Committed. Codex does not load skills, so in Codex sessions the resident one-liners in this file always bind on their own.
...
- Personal overrides live outside the repo: user-level Codex config in `~/.codex/config.toml` and personal global instructions in `~/.codex/AGENTS.md`. Never commit personal config into the repo.

This playbook is itself the guided second-agent port the kit's `okf-second-agent` skill describes (the `.claude/skills/` copy carries the full procedure). The binding rules: this ported playbook replaces `@` imports with explicit session-start reads and states honestly which guardrails are policy-only for Codex (the env-file read denial is mechanical only in Claude Code); the hook copies under `.codex/hooks/` stay byte-identical to `.claude/hooks/` and are declared in the top-level `mirrors:` list in `docs/okf-map.yml` so the safe updater syncs them on kit upgrades; mirrored skills, if ever kept, stay paired by hand; and the parity check lives in this repo's own test gate.
...
- Claude Code's shipped settings mechanically deny reading `.env` files; Codex has no equivalent deny rule wired here, so honor it by policy: never read or print `.env` contents. Never remove, weaken, or work around the Claude-side denial; if a task seems to require reading `.env`, stop and ask me.
